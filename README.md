...
- Актуальный MAX API host: `platform-api2.max.ru` (вместо устаревшего `platform-api.max.ru`).
- В образ включены сертификаты НУЦ Минцифры — без них TLS к `platform-api2.max.ru` падает.
